 ...
 * */
void readStrandsBinaryFie(int L, string path_to_file, vector<vector<char>>& strands, int& M, int& padding_done) {
 ...
 * @param strands - the strands that have been parsed
 * */
void readStrandsBinaryFile(int L, string path_to_file, vector<vector<char>>& strands) {
	ifstream input;
	input.open(path_to_file.c_str());
	if (!input.is_open()) {
		cout << "Failed opening input file!" << endl;
		return;
	}
	string rawStrand;
	while (getline(input, rawStrand)) {
		assert(not rawStrand.empty());
		assert((int )rawStrand.size() == L or (int )rawStrand.size() == L-1);
		vector<char> strand;
		for (auto& chr : rawStrand)
			strand.push_back(chr);
		strands.push_back(strand);
	}
	input.close();
	return;
 ...
 */
void convertToMapOfStrands(vector<vector<char>>& strands_vec, unordered_map<int, vector<int>>& strands_map) {
 ...
 * we've parsed at the end of the run.
 */
void ParseDataToEncoding(string path_to_file, int L, unordered_map<int, vector<int>>& strands, int& padding) {
...
/*! Alex's Addition
 * function that parses the strands data file and create input for the decoding algorithim (top level function)
 * @param path_to_file - path to the data binary file
 * @param L - given strand's length.
 * @param strands - the output strands.
 */
void ParseDataToDecodingAlex(string path_to_file, int L, unordered_map<int, vector<int>>& strands) {

	vector<vector<char>> strands_vec;
	readStrandsBinaryFile(L, path_to_file, strands_vec);
	convertToMapOfStrands(strands_vec, strands);
}

void EncodedStrandsFromFile(string path_to_file, int L, bool bruteForce, unordered_map<int, encoded_strand_binary>& encoded_strands) {
	unordered_map<int, vector<int>> imported_strands;
	ParseDataToDecodingAlex(path_to_file, L, imported_strands);
	for (auto& pr : imported_strands) {
		encoded_strand_binary strand_binary(pr.second, bruteForce);
		pair<int, encoded_strand_binary> newPair(pr.first, strand_binary);
		encoded_strands.insert(newPair);
	}
}
 ...
 * @param new_map - map
 */
void convertUnorderedMapToMap(unordered_map<int, encoded_strand_binary>& encoded_strands,
 ...
 * @param new_map - map
 */
void convertUnorderedMapBeforeEncodingToMap(unordered_map<int, vector<int>>& encoded_strands,
 ...
 */
void exportStrandsToFile(string path_to_file, map<int, encoded_strand_binary> encoded_strands, int padding) {
 ...
 * @param padding - the amount of padding that has been done when reading the file for encoding
 */
void exportStrandsBeforeEncodingToFile(string path_to_file, map<int, vector<int>> strands, int padding) {
	ofstream file(path_to_file, ios::binary);
...
void readEncodedStrandsBinaryFie(string path_to_file, vector<vector<char>>& strands, int L) {
	ifstream file(path_to_file, std::ios::binary);
	assert(file.is_open());
	vector<unsigned char> buffer(istreambuf_iterator<char>(file), { });
	for (int i = 0; i < buffer.size(); i += L) {
 ...
 */
void convertToMapOfEncodedStrands(vector<vector<char>>& strands_vec,
		unordered_map<int, encoded_strand_binary>& strands_map, bool bruteForce) {
 ...
 * @param bruteForce -  boolean denoting wether the w_l part of the encoded data was determined via brute force manner.
 */
void ParseDataToDecoding(const string& path_to_file, int L, unordered_map<int, encoded_strand_binary>& encoded_strands,
		bool bruteForce) {

	vector<vector<char>> strands_vec;
	readEncodedStrandsBinaryFie(path_to_file, strands_vec, L);
	convertToMapOfEncodedStrands(strands_vec, encoded_strands, bruteForce);
