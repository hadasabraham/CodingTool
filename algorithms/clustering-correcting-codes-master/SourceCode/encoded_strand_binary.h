...
class encoded_strand_binary {
	...
	bool brute_force;
public:
	encoded_strand_binary(vector<int> data, bool brute) :
			encoded_data(data), brute_force(brute) {
	}
	;
	 ...
	 */
	void getFirstLogMBits(vector<int>& first_LogM_bits, int index_length) {
	 ...
	 */
	int calculate_wl_size(int M, int e, int t, int index, int (*distanceMetric)(vector<int>, vector<int>)) {
	 ...
	 * @param distanceMetric - the distance metric we're using
	 */
	void getDelta1FromEncodedData(vector<int>& delta_1_from_encoded_data, int M, int e, int t, int index,
		...
		vector<int> output { encoded_data.begin() + w_l_size + index_length, encoded_data.begin() + w_l_size
				+ index_length + delta_1_size };
	 ...
	 */
	void convertDelta1ToInt(set<int>& positions_as_ints, vector<int>& delta1, int position_length) {
	 ...
	 * @param distanceMetric - the distance metric we're using
	 */
	void getDelta2FromEncodedData(vector<int>& delta_2_from_encoded_data, int M, int e, int t, int index,
			int (*distanceMetric)(vector<int>, vector<int>)) {
		...
		vector<int> output { encoded_data.begin() + w_l_size + index_length + delta_1_size, encoded_data.begin()
				+ w_l_size + index_length + delta_1_size + delta_2_size };
	 ...
	 * @param position_length - length of one position's binary representation.
	 */
	void convertDelta2ToInt(set<int>& positions_as_ints, vector<int>& delta2, int position_length) {
	...
	friend bool operator==(const encoded_strand_binary& a, const encoded_strand_binary& b);
};

bool operator==(const encoded_strand_binary& a, const encoded_strand_binary& b) {
	return (a.encoded_data == b.encoded_data) and (a.brute_force == b.brute_force);
}
